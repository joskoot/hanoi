...
;=====================================================================================================
; Hide all except procedure play and parameter idle-limit.
; Prohibits access to all other variables when using DrRacket to run the GUI.

(define-values (play idle-limit)
  ...
  (let ()
    ...
    (define green (make-rgb 0.0 0.8 0.0))(define blue  (make-rgb 0.0 0.0 1.0))


    ;=================================================================================================
    ; Buttons; Are displayed on the screen.
    ; Have has procedure property and can have a content.
      ...
      (let ((no-kontent (string->uninterned-symbol "no-kontent")))
        (λ (stx)
          (syntax-case stx ()
            ...
            ((_ name position kontent) ; Avoiding clash with field content of stuct-type button.
             (let
               ((no-content?
                  (and
                    (identifier? #'kontent)
               ...
               #`(let ((pos position))
                   ...
                   #,@(if no-content?
                        (list)
                        (list #'((draw-button-content viewport) pos kontent)))
                   (λ (action . args)
                     (when
                       (or enabled?
                         (member action
    ...
    (define pos-msg     (add-posn pos-idle    button-width+blok
                          (- (* 2 height-of-button) str-offset)))
    (define pos-warn    (add-posn pos-compute button-width+blok
                          (-      height-of-button  str-offset)))
