...
(define (main)
  (case mode
    ...
    ((long)
     ...
     (long)
     (set! mode 'manual)
     ((draw-button-content vp) mode-pos "manual") (main))
    ((hamilton) (not-yet-implemented 'hamilton) (reset) (main))))
...
(define (long)
  ...
  (let/cc return
    ...
    (define (long conf dest)
      ...
      ; (writeln (list 'long conf dest third))
      (cond
        ((null? conf))
        (else
...
(define (not-yet-implemented node)
  (message-box
    "Mode"
    (format "~s not yet implemented" mode))	
  (reset))
