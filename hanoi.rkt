...
; the GUI and the user. Documentation can be made from module "hanoi.scrbl".
;
;=====================================================================================================
; Prologue
...
(define-syntax-rule
  (define-with-list lst (var value) ...)
  (begin
    (define var value) ...
    (define lst (list var ...))))
...
;=====================================================================================================
; The following two variables are not mutated after receiving their value. The viewport cannot be
; opened yet because the because this needs graphics. Graphics and the viewport are opened by
; procedure initialize, which also initializes variable escape and for which it will be given the
; continuation of of procedure play. The escape is for abort. Used by button quit and when exceeding
; the idle time limit. Will be initialized by procedure initialize.

(define viewport     'delayed)
(define escape       'delayed)

;=====================================================================================================
; Initialization. Store the initial values or restore them when the GUI is started again after a
...
(define (initialize ec)
  ...
  ; Initialize the content of button idle with the current value of parameter idle-limit.
...
;=====================================================================================================
; When the GUI is waiting for a mouse-click or a response on a modal dialog but the user does not
...
; Regions of buttons are included in the buttons themselves.
...
; exception of that of button-idle, whose initial value will be taken from parameter idle-limit by
; procedure initialie. When initializing button idle with (idle-limit), it would receive the default-
; idle-minutes with which parameter idle-limit has been made. Define list of all buttons too. 
...
(define-with-list all-buttons
  ...
  (button-idle    (make-button '|idle limit| pos-idle   'to-be-initialized))
...
; position of procedure dispatch and itself.
...
;=====================================================================================================
; When finishing mode short, long or circular, notify the user. Disable/enable all
; buttons, reset and quit excepted. The latter two allow aborting the short, long and circular action.
...
(define (short)
  ...
  (let/ec ec
    ...
    (define (exit) (clear-move-count-msg) (ec))
    (define p-list
      (for*/list
...
(define (action-setup)
  ...
  (button-cancel 'enable)
  (reset-manual-count)
  (enable/disable-buttons buttons-to-be-disabled 'disable)
  ...
  (enable/disable-buttons buttons-to-be-disabled 'enable)
  (button-cancel 'disable)
  (clear-move-count-msg))
...
(define (action-setup1 disks)
  (unless (null? disks)
    ...
    (dispatch-button pos
      ...
      (button-reset  (clear-move-count-msg) (action-reset))
      (button-cancel (clear-move-count-msg) (action-reset))
      (button-quit   (terminate))
      (else
        (define p (dispatch-peg pos))
        (cond
                  ...
                  (if (eq? SLC 'C)
                    (format
                      "Results for move ~s of path C with ~s disks from peg ~s ~
                       ...
                       Positions of disks in order of increasing size:~n~n~a~n"
...
;=====================================================================================================
; Parallelization of the computation of resulting distribution of disks by action compute.
    
(define (distribute n m) ; Split n in a list of m numbers with sum n. 
  (cond
    ((<= n m) (make-list n 1))
    (else
...
(define (ranges n) ; Split range n in (processor-count) contiguous ranges.
  (define d (distribute n (processor-count)))
  (for/fold ((i 1) (r '()) #:result (reverse r)) ((k (in-list d)))
    (values (+ i k) (cons (list (sub1 i) (+ i k -1)) r))))
    
(define-syntax (posi// stx) ; Does the computations of the postions of the disks parallely.
  (syntax-case stx ()
    ((_ m h f t posi)
     #'(let ()
         (define futures
...
(define (compute-circular h M f t)
  ...
  (cond
    ((= h 1)
     (define r (- 3 f t))
     (case M ; For h=1 things are simple.
       ((1) (values 0 f t (list t)))
       ((2) (values 0 t r (list r)))
       ((3) (values 0 r f (list f)))))
    (else    ; For h>1 things are more complicated.
      (define r (- 3 f t))
      (define h-1 (sub1 h))
      ...
      (define m (modulo (+ M <3^<h-1>-1>/2) 3^h)) ; m is a more comfortable starting point than M.
      (cond                                       ; m is at distribution r...t.
...
(define (clear-move-count-msg) ((clear-string viewport) pos-msg msg-str))

(define (draw-move-count-msg)
  (clear-move-count-msg)
