...
; They are referred to in early stage below. Immutable.
...
;=====================================================================================================
; Computation of the dimensions of buttons. These depend on the sizes of the texts the buttons must
; hold. The sizes are given by procedure get-string-size. A temporary pixmap is used. This requires
; graphics, which is temporarily opened too. Both will be closed after finishing the computation.
...
(define-values (with-of-button height-of-button)
  (let ()
    ...
    (dynamic-wind
      void
      (λ ()
        (for/fold ((w 0) (h 0) #:result (values (+ w 2*str-offset) (+ h 2*str-offset)))
...
(define (action-manual1 d h p) ; Let's see at which peg to put the disk.
  ...
  (define pos (mouse-click-posn (time-out (get-mouse-click viewport))))
  (dispatch-button pos
    (button-peg0 (action-manual2 d h p 0)) ; Yes, peg 0 selected.
    (button-peg1 (action-manual2 d h p 1)) ; Yes, peg 1 selected.
    (button-peg2 (action-manual2 d h p 2)) ; Yes, peg 2 selected.
...
(define buttons-for-action-compute
  (list
    button-height
    button-mode
    button-delay
              ...
              (time-out #t
                (message-box str-compute
                  (if (eq? SLC 'C)
                    ...
                    (format
                      "Results for move ~a of path ~a from peg ~s to peg ~s with ~s disks.~n~n~
